...
$ ansible node1 -m shell -a "echo Hello Clarusway > /home/ec2-user/testfile2 ; cat testfile2"
        #ansible all -m shell -a "ls -l"   ile de bulundugumuz konumdan check edebiliriz. 
```

- Connect to Node1 and show the files and their content. #yeni bir ubuntu instance kur(http and ssh) bu da node3 olsun
...
```bash
[ubuntuserver]
node3 ansible_host=<node3_ip(private olursa degistirmek zorunda kalmazsin)> ansible_user=ubuntu
```
...
$ ansible ubuntuserver -b -m shell -a "apt -y autoremove nginx"
```
